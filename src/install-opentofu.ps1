...
function installStandalone() {
    ...
    try {
        ...
        if ($verifyMethod -eq "gpg")
        {
            ...
            $gpgOutput = & $gpgPath "--no-default-keyring" "--with-colons" "--show-keys" "--fingerprint" "${gpgKeyPath}"
            if (!$?) {
                throw [InstallFailedException]::new("Failed to parse GPG file from ${gpgURL}.")
            }
            $fingerprintLines = $gpgOutput | Select-String -Pattern "^fpr:::::::::${gpgKeyID}:$" | Measure-Object –Line
            if (fingerprintLines -eq 0) {
        ...
        if ($verifyMethod -eq "cosign")
        {
            ...
            $gpgOutput = & $gpgPath --no-default-keyring --keyring $gpgKeyPath --verify $gpgSigPath $sumsPath
            if ($?) {
            ...
            {
                throw [InstallFailedException]::new("Failed to verify ${opentofuVersion} with GPG.")
            }
        } elseif ($verifyMethod -ne "-") {
            throw [InstallFailedException]::new("Bug: unsupported verification method: ${verifyMethod}.")
        }
